...
- `(app visuals) SubOrch#1 [OCC]` — **creative exploration lane only.** Not a build-spec or implementation chat. Raw handoff at `_design\OCC-UI-Visual-Direction-Handoff.txt`; summary at `_system\APP-VISUAL-DIRECTION.md`. Founder direction and distilled principles: `_system\FOUNDER-DIRECTION.md`. Full Living Workspace direction: `_design\OCC-LIVING-WORKSPACE-DIRECTION.md`. **A fresh App Visuals continuation chat should explore visual concepts and concept-image composition first.** It must not generate images or write image-generation prompts in the discussion chat unless Jonathan explicitly asks; once asked, it prepares prompts for separate image-generation chats or tools. This lane is creative exploration only and does not authorize app implementation, router decisions, or architecture changes. B2 is the next router-governance work item and is a separate lane.
